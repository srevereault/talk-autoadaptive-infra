...
class l3_switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      ######## Tipiak Detector ########
      global redcon
      redcon.sadd(a.protosrc, a.protodst)
      if redcon.scard(a.protosrc) > 3:
          log.debug("### Tipiak !!! ")
          log.debug("### On drop le paquet venant de %s" % str(a.protosrc))
          msg = of.ofp_packet_out(in_port = inport, data = event.ofp,
              action = of.ofp_action_output(port = of.OFPP_NONE))
          log.debug("### (Fake) on installe une regle de drop permanente pour %s" % str(a.protosrc))
      else:
      ######## Tipiak Detector ########
